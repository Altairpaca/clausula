...
def normalize_constraints(
    ...
    for index, definition in enumerate(definitions):
        ...
        if kind in {"allowed_instruments", "allowed_sides"}:
            values = [str(item).strip() for item in definition.get("values", ()) if str(item).strip()]
            if not values:
            ...
            normalized["start"] = _clock(definition.get("start"), "trading_window start")
            normalized["end"] = _clock(definition.get("end"), "trading_window end")
...
class ExecutionService:
    ...
    def _versions(self, *, portfolio_id: str | None = None, contract_id: str | None = None):
        ...
        return loader(portfolio_id=portfolio_id, contract_id=contract_id)
    ...
    def _append(
        ...
        *,
        ...
        if writer is None:
            raise ExecutionContractError("repository does not support execution contracts")
        return dict(writer(contract_id, payload))

    def list(self, portfolio_id: str | None = None) -> list[dict[str, Any]]:
        return [dict(row) for row in self._versions(portfolio_id=portfolio_id)]
