        ...
        *,
        ...
        with Path(path).open(newline="", encoding="utf-8-sig") as stream:
            ...
            for row_number, row in enumerate(reader, 2):
                try:
                    observed_at = canonical_timestamp(row.get("observed_at") or row.get("date") or "")
                    ...
                    identifier = (row.get("ticker") or row.get("instrument") or "").strip()
                    if not identifier:
                    ...
                    identifier_scheme = (row.get("identifier_scheme") or "ticker").strip()
        ...
        for row in rows:
            instrument_id = self.repository.instrument(
                InstrumentIdentifier(
                    row["identifier"], row["identifier_scheme"]
                ),
                row["instrument_name"],
                row["asset_type"],
                row["currency"],
            ...
            for row_number, row in enumerate(reader, 2):
                try:
                    ...
                    if not from_currency or not to_currency or from_currency == to_currency:
                        raise ValueError("distinct from_currency and to_currency are required")
                    if dec(rate) <= 0:
    ...
    def fx_rate(
        ...
        *,
        ...
        row = self.repository.market_fx_rate(
            from_currency, to_currency, as_of, known_as_of, dataset_name, dataset_version
        )
        if row is None:
            raise KeyError(f"no accepted FX rate for {from_currency}/{to_currency} at {as_of}")
        return row
